...
class Blockchain {
  ...
  /**
   * Adds a new block to the non-empty blockchain and returns it.
   ...
   * If the block body contains 'star', decodes the star's story.
  ...
  async getBlock(blockHeight) {
    this._checkIfInitialized();
    let block = JSON.parse(await this._db.get(blockHeight.toString()));
  ...
  /**
   * Returns an array of blocks having the given Bitcoin address.
   *
   * @param {string} address Bitcoin address.
   * @returns {Array.Block} Array of blocks having the given Bitcoin address.
   * @memberof Blockchain
   */
  async getBlocksByAddress(address) {
    ...
    for (var blockHeight = 0; blockHeight < blockCount; blockHeight++) {
      let block = await this.getBlock(blockHeight);
      if (block['body']['address'] && (block['body']['address'] === address)) {
        blocks.push(block);
      }
    }
    return blocks;
  ...
  /**
   * Returns a block with the given hash or null if there is no such block.
   *
   * @param {string} hash Block hash.
   * @returns {Block} Block with the given hash or null.
   * @memberof Blockchain
   */
