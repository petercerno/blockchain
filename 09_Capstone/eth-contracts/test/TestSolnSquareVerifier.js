...
contract('TestSolnSquareVerifier', accounts => {
  ...
  describe('SolnSquareVerifier Tests', () => {
    ...
    before('contract setup', async () => {
      solnSquareVerifier = await SolnSquareVerifier.new({ from: ownerAccount });
      squareVerifier = await SquareVerifier.new({ from: ownerAccount });
      await solnSquareVerifier.setVerifier(
        squareVerifier.address, { from: ownerAccount });
    });
    ...
    it('new solution can be added', async () => {
      ...
      await solnSquareVerifier.submitSolution(
        A, A_p, B, B_p, C, C_p, H, K, input, { from: userAccount1 });

      const events = await solnSquareVerifier.getPastEvents(
        'SolutionSubmitted');

      const eventSolutionSubmittedEmitted = (events.length == 1);
      assert.isTrue(eventSolutionSubmittedEmitted,
        'SolutionSubmitted event not emitted');
      assert.equal(events[0].args.user, userAccount1,
        'Invalid event user account');

      // Cannot submit the same solution.
      await expectThrow(solnSquareVerifier.submitSolution(
        A, A_p, B, B_p, C, C_p, H, K, input, { from: userAccount2 }));
    });

    it('cannot submit invalid solution', async () => {
      // This proof was generated by running (using zokrates:0.3.3):
    ...
    it('new ERC721 token can be minted', async () => {
      await solnSquareVerifier.mint(userAccount1, 1, { from: ownerAccount });

      const events = await solnSquareVerifier.getPastEvents('Transfer');
      ...
      assert.equal(events[0].args.to, userAccount1, 'Invalid to');
      assert.equal(toNumber(events[0].args.tokenId), 1, 'Invalid tokenId');

      // Cannot mint another token.
      await expectThrow(
        solnSquareVerifier.mint(userAccount1, 2, { from: ownerAccount }));
      // Cannot mint without any submitted solution.
      await expectThrow(
        solnSquareVerifier.mint(userAccount2, 2, { from: ownerAccount }));
    });
